var Prefs = (function () {
  // @include "config/PREF_TO_FILTER.js"

  // @include "lib/ArrayEx.js"
  // @include "lib/FileEx.js"
  // @include "lib/FolderEx.js"
  // @include "lib/ObjectEx.js"
  // @include "lib/Os.js"
  // @include "lib/StringEx.js"

  var module = {};

  /**
   * @param {boolean} [isBeta]
   * @returns {string}
   */
  module.getAppName = function (isBeta) {
    isBeta = setValueIfUndefined(isBeta, app.isBeta);
    return isBeta ? 'After Effects (Beta)' : 'After Effects';
  };

  /**
   * @param {string} [appVersion]
   * @returns {string}
   */
  module.getAppVersion = function (appVersion) {
    appVersion = setValueIfUndefined(appVersion, app.version);

    var version = appVersion.split('x')[0];
    if (version === '22.6.4') return version; // One time bug introduced by Adobe

    var items = version.split('.');
    var major = items[0];
    var minor = items[1] || 0;
    var patch = items[2] || 0;

    return [major, minor].join('.');
  };

  /**
   * @param {string} PREFType
   * @returns {File}
   */
  module.getFile = function (PREFType) {
    app.preferences.saveToDisk();

    var files = module.getFiles(PREFType);
    files.sort(sortByDate);

    return files[0];
  };

  /**
   * @param {string} PREFType
   * @returns {File[]}
   * @throws {Error}
   */
  module.getFiles = function (PREFType) {
    var folder = module.getFolder();
    if (!folder.exists) {
      throw new Error(
        'Preferences folder does not exist at path ' + folder.fsName
      );
    }

    var filter = getFilter(PREFType);
    var files = folder.getFiles(filter);
    if (files.length === 0) {
      throw new Error(
        'Could not find files with filter "' +
          filter +
          '" in Preferences folder at path ' +
          folder.fsName
      );
    }

    return files;
  };

  /**
   * @param {boolean} [isWindows]
   * @param {boolean} [isBeta]
   * @param {string} [appVersion]
   * @returns {Folder}
   */
  module.getFolder = function (isWindows, isBeta, appVersion) {
    var rootFolder = module.getRootFolder(isWindows);
    var appName = module.getAppName(isBeta);
    var version = module.getAppVersion(appVersion);

    return FolderEx.getFolderObject(
      rootFolder.fsName + '/Adobe/' + appName + '/' + version
    );
  };

  /**
   * @param {string} PREFType
   * @returns {Object}
   */
  module.getJson = function (PREFType) {
    return toJson(FileEx.readFile(module.getFile(PREFType)));
  };

  /**
   * @param {boolean} [isWindows]
   * @returns {Folder}
   */
  module.getRootFolder = function (isWindows) {
    isWindows = setValueIfUndefined(isWindows, Os.isWindows());

    return FolderEx.getFolderObject(
      isWindows
        ? Folder.userData.fsName
        : Folder.userData.parent.fsName + '/Preferences'
    );
  };

  /**
   * @returns {string[]}
   */
  module.getSupportedPrefTypes = function () {
    return ObjectEx.keys(PREF_TO_FILTER);
  };

  return module;

  ///

  function getFilter(PREFType) {
    PREFType = PREFType || 'PREF_Type_MACHINE_SPECIFIC';

    if (!PREF_TO_FILTER.hasOwnProperty(PREFType)) {
      throw new Error('PREFType ' + PREFType + ' does not exist');
    }

    return PREF_TO_FILTER[PREFType];
  }

  function isComment(string) {
    return StringEx.startsWith(string, '#');
  }

  function isSection(string) {
    return StringEx.startsWith(string, '[') && StringEx.endsWith(string, ']');
  }

  function joinMultiline(string) {
    return string.replace(/"*\\[\n|\r]\s*"?/g, '');
  }

  function setValueIfUndefined(value, fallbackValue) {
    return typeof value === 'undefined' ? fallbackValue : value;
  }

  function sortByDate(f1, f2) {
    return f1.modified < f2.modified;
  }

  function toJson(string) {
    var json = {};
    var section;
    StringEx.forEachLine(joinMultiline(string), function (line) {
      line = StringEx.trim(line);

      if (isComment(line) || !line) return;
      if (isSection(line)) {
        section = line.replace(/["\[\]]/g, '');
        json[section] = {};
      } else {
        if (!section) throw new Error('Section not found');

        var match = line.match(/^(".*?")( = )(.*)/);
        if (!match || match.length !== 4) {
          throw new Error('Could not parse key-value from line\n' + line);
        }

        var key = StringEx.unquote(match[1]);
        var value = StringEx.unquote(match[3]);

        json[section][key] = value;
      }
    });
    return json;
  }
})();
